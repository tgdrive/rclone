...
// TestMain (re)creates the test schema once before all tests.
...
// newStorage creates a Storage connected to the test schema with the default
// config_name ("main"). Tables are truncated after the test via t.Cleanup so
// each test gets a clean slate while still allowing multi-Storage round trips
// within a single test.
func newStorage(t *testing.T, ctx context.Context) *Storage {
	t.Helper()
	return newStorageWithConfig(t, ctx, defaultConfigName)
}

// newStorageWithConfig is like newStorage but sets the given config_name.
func newStorageWithConfig(t *testing.T, ctx context.Context, configName string) *Storage {
	t.Helper()
	dbURL := testDBURL
	if e := os.Getenv("RCLONE_TEST_DB"); e != "" {
		dbURL = e
...
func TestConfigNameDefaultAndCustom(t *testing.T) {
	ctx := context.Background()
	defer withoutPassword(t)()

	// Two storages in the same schema: default ("main") and custom ("work")
	sMain := newStorage(t, ctx)
	sWork := newStorageWithConfig(t, ctx, "work")
	...
	sMain.SetValue("remote", "type", "drive")
	sMain.SetValue("remote", "client_id", "main-client")
	sWork.SetValue("remote", "type", "s3")
	sWork.SetValue("remote", "provider", "AWS")
	...
	// Reload default config — should only see default config's data
	sMain2 := newStorage(t, ctx)
	err = sMain2.Load()
	require.NoError(t, err)
	v, found := sMain2.GetValue("remote", "type")
	assert.True(t, found)
	assert.Equal(t, "drive", v)
	v, found = sMain2.GetValue("remote", "client_id")
	assert.True(t, found)
	assert.Equal(t, "main-client", v)
	_, found = sMain2.GetValue("remote", "provider")
	assert.False(t, found, "work config data should not leak into default config")

	// Reload work config — should only see work config's data
	sWork2 := newStorageWithConfig(t, ctx, "work")
	err = sWork2.Load()
	require.NoError(t, err)
	v, found = sWork2.GetValue("remote", "type")
	assert.True(t, found)
	assert.Equal(t, "s3", v)
	v, found = sWork2.GetValue("remote", "provider")
	assert.True(t, found)
	assert.Equal(t, "AWS", v)
	_, found = sWork2.GetValue("remote", "client_id")
	assert.False(t, found, "default config data should not leak into work config")
}

func TestConfigNameEncryptionIsolation(t *testing.T) {
	ctx := context.Background()

	// Set password for first config
	cleanupA := withPassword(t, "password-a")
	defer cleanupA()
	sA := newStorageWithConfig(t, ctx, "configA")
	err := sA.Load()
	require.NoError(t, err)
	sA.SetValue("remoteA", "type", "drive")
	sA.SetValue("remoteA", "token", "secret-a")
	err = sA.Save()
	require.NoError(t, err)
	config.ClearConfigPassword()

	// Set a different password for second config
	cleanupB := withPassword(t, "password-b")
	defer cleanupB()
	sB := newStorageWithConfig(t, ctx, "configB")
	err = sB.Load()
	require.NoError(t, err)
	sB.SetValue("remoteB", "type", "s3")
	sB.SetValue("remoteB", "token", "secret-b")
	err = sB.Save()
	require.NoError(t, err)
	config.ClearConfigPassword()

	// Verify blob table has two entries (one per config_name)
	var count int
	err = sA.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM rclone_config_encrypted").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 2, count)

	// Reload configA with its password — should only see configA data
	err = config.SetConfigPassword("password-a")
	require.NoError(t, err)
	defer config.ClearConfigPassword()

	sA2 := newStorageWithConfig(t, ctx, "configA")
	err = sA2.Load()
	require.NoError(t, err)
	v, found := sA2.GetValue("remoteA", "type")
	assert.True(t, found)
	assert.Equal(t, "drive", v)
	v, found = sA2.GetValue("remoteA", "token")
	assert.True(t, found)
	assert.Equal(t, "secret-a", v)
	_, found = sA2.GetValue("remoteB", "token")
	assert.False(t, found, "configB data should not leak into configA")
}

func TestConfigNameRoundTripAfterSchemaReset(t *testing.T) {
	// This test ensures that config_name != "" data survives a schema-level
	// truncate (simulating schema recreation) and correctly round-trips.
	ctx := context.Background()
	defer withoutPassword(t)()

	// Use a dedicated schema for this test
	schemaURL := rewriteSearchPath(testDBURL, "configpg_test_reset")
	rawDB, err := openPG(rewriteSearchPath(testDBURL, "public"))
	require.NoError(t, err)
	defer rawDB.Close()
	_, _ = rawDB.ExecContext(ctx, "DROP SCHEMA IF EXISTS configpg_test_reset CASCADE")

	s, err := NewStorage(ctx, schemaURL+"&config_name=persistent")
	require.NoError(t, err)
	s.SetValue("data", "key", "value")
	err = s.Save()
	require.NoError(t, err)
	s.Close()

	// Simulate schema drop + recreate (as in migration)
	_, _ = rawDB.ExecContext(ctx, "DROP SCHEMA IF EXISTS configpg_test_reset CASCADE")
	s2, err := NewStorage(ctx, schemaURL+"&config_name=persistent")
	require.NoError(t, err)
	defer func() { _ = s2.Close() }()

	err = s2.Load()
	require.NoError(t, err)
	// After schema drop, the data is gone
	assert.Equal(t, []string{}, s2.GetSectionList())

	// Write new data
	s2.SetValue("newdata", "key", "newvalue")
	err = s2.Save()
	require.NoError(t, err)

	s3, err := NewStorage(ctx, schemaURL+"&config_name=persistent")
	require.NoError(t, err)
	defer func() { _ = s3.Close() }()

	err = s3.Load()
	require.NoError(t, err)
	v, found := s3.GetValue("newdata", "key")
	assert.True(t, found)
